...
SELECT 
    ROUND(AVG(sum_quantity)) AS daily_orders
FROM
    (SELECT 
...
SELECT 
		pizza_types.category as category,
        ROUND((sum(order_details.quantity * pizzas.price) / (SELECT  ROUND(SUM(order_details.quantity * pizzas.price),2) AS total_price
		FROM order_details
